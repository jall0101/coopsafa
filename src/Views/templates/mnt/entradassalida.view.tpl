    ...
    <form action="index.php?page=Mnt_Entradassalida&mode={{mode}}&idEntradas_salidas={{idEntradas_salidas}}" method="POST"
      class="col-10 align-self-center bg-gris_claro p-4 rounded" enctype="multipart/form-data">
        ...
        <section class="row mb-3 bg-gris_claro2 p-3 rounded">
          <label for="idEntradas_salidas" class="col-4 form-label">Código</label>
          <input type="hidden" id="idEntradas_salidas" name="idEntradas_salidas" value="{{idEntradas_salidas}}" />
          <input type="hidden" id="mode" name="mode" value="{{mode}}" />
          <input type="hidden" name="xssToken" value="{{xssToken}}">
          <input type="text" readonly class="form-control" name="idEntradas_salidasdummy" value="{{idEntradas_salidas}}" />
        </section>
